...
def session_factory(user_agent=None):
    ...
    session.headers["User-Agent"] = user_agent or random.choice(
        _util.USER_AGENTS)
    return session
...
class State(object):
    ...
    def login(cls, email, password, on_2fa_callback, user_agent=None):
        ...
        r = session.post(
            "https://m.facebook.com/login.php?login_attempt=1", data=data)
    ...
    def logout(self):
        ...
        if not logout_h:
            ...
            logout_h = re.search(
                r'name=\\"h\\" value=\\"(.*?)\\"', h_r.text).group(1)
    ...
    def from_session(cls, session):
        ...
        revision = int(r.text.split('"client_revision":', 1)
                       [1].split(",", 1)[0])
